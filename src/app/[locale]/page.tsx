    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-brand-300 via-brand-400 to-brand-600 px-4 pb-24 pt-14 sm:px-6 sm:pt-20 lg:px-8">
        ...
        <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-8">
          <div className="relative z-10 max-w-xl">
            ...
            <p className="mt-6 max-w-md text-lg leading-relaxed text-brand-950/80">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4">
              ...
              <a
                href="#how-it-works"
                className="flex items-center gap-2 rounded-full bg-white/70 px-5 py-3.5 text-sm font-semibold text-brand-950 backdrop-blur transition-colors hover:bg-white"
              >
          ...
          <div className="relative hidden min-h-[380px] lg:block">
            <div
              className="animate-float-slow absolute right-24 top-2 flex h-40 w-40 rotate-[-6deg] items-center justify-center rounded-[2rem] bg-white text-7xl shadow-xl"
              style={{ ["--rot" as string]: "-6deg" }}
            >
              🗼
            </div>
            <div
              className="animate-float-slow-delay absolute right-0 top-40 flex h-32 w-32 rotate-[5deg] items-center justify-center rounded-[1.75rem] bg-white text-5xl shadow-xl"
              style={{ ["--rot" as string]: "5deg" }}
            >
              🕌
            </div>
            <div
              className="animate-float-slow absolute left-4 top-56 flex h-28 w-28 rotate-[-4deg] items-center justify-center rounded-[1.5rem] bg-white text-4xl shadow-xl"
              style={{ ["--rot" as string]: "-4deg" }}
            >
              🏛️
            </div>

            <div className="absolute left-0 top-16 flex w-64 items-center gap-3 rounded-2xl bg-white p-3 shadow-lg">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-100 text-lg">
                🧭
              </div>
              <div className="min-w-0">
                <p className="truncate text-xs font-semibold text-brand-950">{dict.home.hero.chip1Title}</p>
                <p className="truncate text-[11px] text-brand-950/50">{dict.home.hero.chip1Sub}</p>
              </div>
            </div>

            <div className="absolute bottom-4 right-16 flex w-60 items-center gap-3 rounded-2xl bg-white p-3 shadow-lg">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-coral-500/15 text-lg">
                🔔
              </div>
              <div className="min-w-0">
                <p className="truncate text-xs font-semibold text-brand-950">{dict.home.hero.chip2Title}</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-base text-brand-950/60">{dict.home.features.subtitle}</p>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-3">
            {dict.home.how.steps.map((step, i) => (
              <div key={step.title} className="relative rounded-2xl bg-white p-6">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-950 text-sm font-bold text-white">
                  {i + 1}
                </div>
                <h3 className="mt-4 text-base font-semibold text-brand-950">{step.title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center justify-between gap-6 rounded-3xl bg-brand-950 px-8 py-10 text-center sm:flex-row sm:text-left">
          ...
          <Link
            href={`/${l}/pricing`}
            className="shrink-0 rounded-full bg-white px-6 py-3 text-sm font-semibold text-brand-950 transition-transform hover:scale-[1.03]"
          >
      ...
      <section className="mx-auto max-w-3xl px-4 pb-24 text-center sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold tracking-tight text-brand-950">{dict.home.finalCta.title}</h2>
        <p className="mt-3 text-base text-brand-950/60">{dict.home.finalCta.subtitle}</p>
        <Link
          href={`/${l}/planner`}
