...
export function CitySearchInput({
  ...
  return (
    <div ref={rootRef} className="relative flex flex-1 flex-col gap-2 text-sm font-medium text-brand-950">
      {label}
      <input
        value={query}
        onChange={(e) => {
      ...
      />
      {open && (
        <div className="absolute top-full z-50 mt-1 max-h-80 w-full overflow-y-auto rounded-2xl border border-black/5 bg-white py-2 shadow-xl">
            ...
            <div>
              ...
              {curatedMatches.map((c) => (
                <button
                  ...
                  className="flex w-full items-center gap-2 px-3.5 py-2 text-start text-sm text-brand-950/80 hover:bg-sand-50"
                >
                  <span>{c.emoji}</span>
                  {isTr ? c.cityTr : c.cityEn}
                  <span className="text-xs text-brand-950/40">{isTr ? c.countryTr : c.countryEn}</span>
                </button>
              ))}
            </div>
          )}
          ...
          {query.trim().length >= 2 && (
            <div>
              ...
              {liveResults
                ...
                .map((r) => (
                  <button
                    ...
                    className="flex w-full flex-col px-3.5 py-2 text-start text-sm text-brand-950/80 hover:bg-sand-50"
                  >
                    <span>📍 {r.name}</span>
              ...
              {!loading && liveResults.length === 0 && curatedMatches.length === 0 && (
                <div className="px-3.5 py-2 text-sm text-brand-950/40">{noResultsLabel}</div>
              )}
            </div>
          )}
        </div>
